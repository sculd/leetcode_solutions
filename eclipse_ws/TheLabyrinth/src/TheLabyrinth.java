			...
			for (int r = m - 1; r >= 0; r--)	{
				for (int c = n - 1; c >= 0; c--)	{
					...
					if (area == '2')	
						...
						if (r < m-1 && grid[f][r+1][c] != 'o')	{
							long temp_dist = (long)1 + dists[f][r+1][c];
							dist = Math.min(dist, temp_dist);
							if (dist == temp_dist)
								dir = Direction.SOUTH;
						}
						if (r > 0 && grid[f][r-1][c] != 'o')	{
							long temp_dist = (long)1 + dists[f][r-1][c];
							dist = Math.min(dist, temp_dist);
							if (dist == temp_dist)
								dir = Direction.NORTH;
						}
						if (c < n-1 && grid[f][r][c+1] != 'o')	{
							long temp_dist = (long)1 + dists[f][r][c+1];
							dist = Math.min(dist, temp_dist);
							if (dist == temp_dist)
								dir = Direction.EAST;
						}
						if (c > 0 && grid[f][r][c-1] != 'o')	{
							long temp_dist = (long)1 + dists[f][r][c-1];
							dist = Math.min(dist, temp_dist);
							if (dist == temp_dist)
								dir = Direction.WEST;
			...
			for (int c = n - 1; c >= 0; c--)	{
				if (grid[0][r][c] == '1')	{
					...
					for (int s = 0; s < dist; s++)	{
						Direction dir = directions[i][j][k];
						sn.directions[s] = dir;
